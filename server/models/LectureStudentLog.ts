...
export default class LectureStudentLog {
  ...
  getStudent(lecture_uid: string, account_uid: string) {
    return this.mysql.query('SELECT elapsed, status FROM lecture_student_log WHERE lecture_uid = ? and account_uid = ?', [lecture_uid, account_uid]);
  }
