...
export function getPresentnessAndUnderstandingScores(lecture: Lecture, log: Status[], scoreLog: Score[]) {
  ...
  console.log(log, scoreLog);
  // perform correctness check (at least at the beginning)
  let badUids = getIncorrect(log, scoreLog);
  if (badUids.size !== 0) {
    console.error(Date.now(), 'present/avg us error for lecture_uid', lecture.uid, 'with uids', badUids);
  ...
  for (let each of log) {
    if (!logMap[each.account_uid])
      logMap[each.account_uid] = [];
  ...
  for (let each in logMap) {
    if (logMap[each].length % 2 !== 0) {
      console.error(Date.now(), '(1) present parse error for lecture_uid', lecture.uid, 'and uid', each);
      logMap[each].push({
        account_uid: each,
        elapsed: lectureLength,
        status: 'l'
      });
    }
  ...
  for (let each of scoreLog) {
    if (!scoreLogMap[each.account_uid])
      scoreLogMap[each.account_uid] = [];
