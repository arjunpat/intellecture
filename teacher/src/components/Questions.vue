          ...
          <v-row align="center" justify="center">
            ...
            <v-row>
                ...
                >
                    ...
                    >
                        <v-list-item-content>
                        <v-list-item-title><div style="display: inline-block; font-size: 20px;" class="topic" @click="$emit('showCategory',n-1)"><span v-if="topics[n-1].value.length > 20">{{ topics[n-1].value.substring(0, 20) }}...</span><span v-else>{{ topics[n-1].value }}</span></div><div id="topic-quantity" class="ml-3">{{ topics[n-1].questions.length }}</div></v-list-item-title>
                        </v-list-item-content>
                    </v-list-item>
                    <!-- EXAMPLE TOPIC -->
                    <TutorialDisplay :show="showTutorial == 4" backgroundColor="white" @next="nextTutorial()" @cancel="resetTutorial()" bottom>
                ...
                <ul style="list-style-type: none; font-family: var(--main-font);">
                    ...
                    <!-- EXAMPLE QUESTION -->
                    <TutorialDisplay :show="showTutorial == 5" backgroundColor="white" @next="nextTutorial(); clickTab(2);" @cancel="resetTutorial()" bottom>
                      <template v-slot:title>
                        Questions
                      </template>
                      <template v-slot:explanation>
