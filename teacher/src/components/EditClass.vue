  ...
  <v-row>
    <v-dialog v-model="dialog" persistent max-width="600px">
      <template v-slot:activator="{ on }">
        <v-btn text color="error" v-on="on" style="font-family: 'Poppins'; font-size: 15px;" v-if="remove">Delete</v-btn>
        <v-btn text color="dark" v-on="on" style="font-family: 'Poppins'; font-size: 15px;" v-else>Rename</v-btn>
      </template>
      <v-card>
          <v-container>
